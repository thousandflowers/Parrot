# RefineClone — Piano Completo di Miglioramento
...
- [ ] **Step 2: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter MockAXBridgeTests/testExtract_whenPIDIsZero_throwsNoTextSelected 2>&1 | tail -5
```
...
- [ ] **Step 4: Build**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift build 2>&1 | tail -5
```
...
- [ ] **Step 3: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter ResultCacheTests/testSet_updatingExistingKey_doesNotDoublecountMemory 2>&1 | tail -5
```
...
- [ ] **Step 5: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter ResultCacheTests 2>&1 | tail -10
```
...
Fix: usare `Logger` (iOS 14+ / macOS 11+) con subsystem `com.thousandflowers.refineclone` e categoria per file.
...
- [ ] **Step 2: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter FeedbackLoggerTests 2>&1 | tail -5
```
Expected: PASS.
...
- [ ] **Step 4: Build + test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift build 2>&1 | tail -5 && swift test --filter FeedbackLoggerTests 2>&1 | tail -5
```
...
- [ ] **Step 2: Build**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift build 2>&1 | tail -10
```
...
---
name: Bug report
about: Segnala un bug in RefineClone
labels: bug
---
...
**Screenshot / log**
Se possibile, allega screenshot o log da Console.app (filtro: `com.thousandflowers.refineclone`).

**Ambiente**
- macOS: [es. 14.5]
- CPU: [Apple Silicon / Intel]
- Versione RefineClone: [es. 1.0.0]
...
- [ ] **Step 2: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter CorrectionResultTests/testComputeDiff_consecutiveSpaces_correctOffsets 2>&1 | tail -5
```
...
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter CorrectionResultTests 2>&1 | tail -10
```
Expected: tutti PASS.
...
- [ ] **Step 3: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter LLMAPITypesTests 2>&1 | tail -5
```
Expected: PASS.
...
- [ ] **Step 7: Build + test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift build 2>&1 | tail -10 && swift test 2>&1 | tail -10
```
...
- [ ] **Step 4: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter PromptEngineTests/testBuildTranslationPrompt_containsTargetLanguage 2>&1 | tail -5
```
Expected: PASS.
...
- [ ] **Step 7: Build + test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift build 2>&1 | tail -5 && swift test 2>&1 | tail -5
```
...
- [ ] **Step 2: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter DiffHighlightTests 2>&1 | tail -5
```
Expected: PASS.
...
- [ ] **Step 4: Run test**
```bash
cd /Users/eugeniozamengopontrelli/RefineClone && swift test --filter HistoryStoreTests 2>&1 | tail -5
```
Expected: PASS.
...
- Sezione "Why RefineClone" con 3-4 differenziatori
- Benchmarks: "Corregge testo in 2-5s su M2 con modello Qwen 2.5 1.5B"
- Sezione "Install" con DMG download e `brew install --cask refineclone`
...
| Feature | RefineClone | Grambo | Stanza |
|---|---|---|---|
...
## Quick Start
1. Launch RefineClone — it appears in your menu bar
...
## Contributing
Found a bug? [Open an issue](https://github.com/thousandflowers/refineclone/issues/new/choose).
```
...
```xml
<key>SUFeedURL</key>
<string>https://raw.githubusercontent.com/thousandflowers/refineclone/main/appcast.xml</string>
```
...
```bash
cat -n /Users/eugeniozamengopontrelli/RefineClone/UI/AppRulesTab.swift
```
